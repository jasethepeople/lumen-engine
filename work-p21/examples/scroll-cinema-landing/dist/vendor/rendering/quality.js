...
const MSAA_STEPS = [8, 4, 2, 0];
function postCount(rung, configured) {
 ...
 */
export function buildLadder(base = LADDER_V1, configuredPostPasses = 0) {
    ...
    for (let i = 0; i < base.length; i += 1) {
        ...
        for (let k = shed.length - 1; k >= 0; k -= 1)
            out.push(shed[k]);
    }
    return out;
}
export class AdaptiveQualityController {
    ...
    lastStepAt = 0;
    headroomStreak = 0;
    constructor(opts = {}, initialPostPasses = []) {
    ...
    update(stats, nowMs = now()) {
        ...
        if (headroom) {
            ...
            if (this.headroomStreak >= this.upStreakRequired &&
    ...
    getLevel() {
        ...
        const level = { dprScale: rung.dprScale, msaa: rung.msaa, postPasses };
        if (rung.shadowMapSize !== undefined)
            level.shadowMapSize = rung.shadowMapSize;
        return level;
    }
