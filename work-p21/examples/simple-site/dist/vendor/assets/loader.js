...
async function fetchBytes(env, url) {
    const init = {};
    if (env.signal)
        init.signal = env.signal;
...
function pickImageUrl(entry) {
    const srcset = entry.variants.avif?.srcset ?? entry.variants.webp?.srcset ?? null;
    if (srcset) {
...
async function importHls() {
    try {
        ...
        if (Hls && typeof Hls === 'function') {
            const supported = Hls.isSupported?.() ?? true;
            return supported ? Hls : null;
        }
        return null;
    }
    catch {
...
async function loadVideo(entry, opts) {
    ...
    if (entry.irVariants && entry.irVariants.length > 0 && opts.capabilities) {
        ...
        if (pick) {
            if (pick.delivery === 'hls')
                rawHls = pick.src;
            else
                rawProgressive = pick.src;
        }
    }
    const progressive = typeof rawProgressive === 'string' && rawProgressive !== '' ? rawProgressive : undefined;
    const hlsPlaylist = typeof rawHls === 'string' && rawHls !== ''
        ? rawHls
        : undefined;
    if (entry.scrubOptimized && progressive) {
        ...
        if (HlsCtor) {
            hls = new HlsCtor();
            usedHls = true;
            url = resolveAssetUrl(hlsPlaylist, opts.cdnBase);
        }
        else {
            url = progressive ?? resolveAssetUrl(hlsPlaylist, opts.cdnBase);
        }
    }
    else {
        url = hlsPlaylist ? resolveAssetUrl(hlsPlaylist, opts.cdnBase) : progressive;
    }
    ...
    if (entry.poster)
        el.poster = resolveAssetUrl(entry.poster, opts.cdnBase);
...
/* --------------------------------------------------------------- loaders */
async function loadImage(entry, opts) {
...
async function loadModel(entry, opts) {
    const env = makeEnv(opts);
...
async function loadFont(entry, opts) {
    const env = makeEnv(opts);
...
export async function loadAsset(entry, opts = {}) {
    const emit = opts.onState ?? (() => undefined);
    emit('queued');
    emit('loading');
    try {
        let handle;
