...
/**
 * Read optional parallax scale endpoints from a stage node's meta
 * (`meta.parallax: {from, to}`); defaults to PARALLAX_SCALE (1.0 -> 1.08).
 */
function parallaxScaleFromMeta(meta: Record<string, unknown> | undefined): [number, number] {
  const p = meta?.['parallax'];
  ...
  return [PARALLAX_SCALE[0], PARALLAX_SCALE[1]];
}

/**
 * Poster fallback for the stage video plane: an explicit image asset id in
 * the stage node's meta (`meta.poster`) wins; otherwise the manifest video
 * entry's poster frame URL is used. Returns undefined when neither exists.
 */
function posterFallback(
  manifest: AssetManifest,
  stageMeta: Record<string, unknown> | undefined,
  videoAsset: AssetManifest['assets'][string] | undefined,
): string | undefined {
  const metaPoster = stageMeta?.['poster'];
  if (typeof metaPoster === 'string' && metaPoster !== '') {
    const entry = manifestEntry(manifest, metaPoster);
    if (entry?.kind === 'image') return entry.variants.fallback.url;
  }
  return videoAsset?.kind === 'video' && videoAsset.poster !== '' ? videoAsset.poster : undefined;
}
...
function composeScrollCinemaLanding(cfg: EngineConfig, manifest: AssetManifest): ComposedScene {
  ...
  // Parallax endpoints: stage node meta.parallax {from,to} overrides defaults.
  const parallaxScale = parallaxScaleFromMeta(videoRef?.meta);
  const poster = posterFallback(manifest, videoRef?.meta, videoAsset);
  ...
  tracks.push(
    makeTrack(parallaxTrackId, videoNodeId, 'scroll', [0, totalRange], [
      { t: 0, value: parallaxScale[0], easing: 'linear' },
      { t: totalRange, value: parallaxScale[1], easing: 'linear' },
    ]),
  );
  ...
  chapterScenes.forEach((scene, i) => {
    ...
    // Clamp into the real scroll extent (epsilon=0: raw clamp+swap so
    // degenerate/fully-outside explicit ranges are still detectable and
    // fall back to the computed slice), then enforce the min window width.
    let [start, end] = explicit
      ? normalizeScrollRange(explicit[0], explicit[1], totalRange, 0)
      : [innerStart + i * slice, innerStart + (i + 1) * slice];
    if (end - start <= 0) {
      [start, end] = normalizeScrollRange(innerStart + i * slice, innerStart + (i + 1) * slice, totalRange);
    }
