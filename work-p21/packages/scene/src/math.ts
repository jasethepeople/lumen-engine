...
import type { Quat, Transform, Vec2, Vec3 } from '@lumen/contracts';
...
export function lerpArray(a: readonly number[], b: readonly number[], t: number): number[] {
  ...
  for (let i = 0; i < a.length; i++) out[i] = lerp(a[i]!, b[i] ?? a[i]!, t);
  return out;
}
...
export function transformsEqual(a: Transform, b: Transform, eps = 1e-9): boolean {
  for (let i = 0; i < 3; i++) {
    if (Math.abs(a.position[i]! - b.position[i]!) > eps) return false;
    if (Math.abs(a.scale[i]! - b.scale[i]!) > eps) return false;
  }
  for (let i = 0; i < 4; i++) {
    if (Math.abs(a.rotationQuat[i]! - b.rotationQuat[i]!) > eps) return false;
  }
  return true;
}
