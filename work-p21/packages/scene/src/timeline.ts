...
import type { CubicBezier, EasingName, Keyframe, TimelineTrack } from '@lumen/contracts';
import { lerp, lerpArray } from './math.js';

/** Track value type (matches Keyframe.value). */
export type TrackValue = number | number[] | string;

/** How the playhead wraps at the ends of track.range. */
 ...
 */
export function cubicBezierEase(bezier: CubicBezier, t: number): number {
...
function isCubicBezier(e: EasingName | CubicBezier): e is CubicBezier {
  return Array.isArray(e);
}

/** Apply an easing (named or cubic bezier) to normalized progress t in [0, 1]. */
export function applyEasing(easing: EasingName | CubicBezier, t: number): number {
...
/** Options for evaluating a track at a raw playhead position. */
export interface EvaluateOptions {
  /** Loop behavior within track.range. Defaults to 'none' (clamp). */
  loop?: LoopMode;
  /** Easing override applied between keyframes (overrides Keyframe.easing). */
  easing?: EasingName | CubicBezier;
}

/** Map a raw playhead position onto the track range honoring the loop mode. */
export function resolvePlayhead(raw: number, range: readonly [number, number], loop: LoopMode): number {
  const [start, end] = range;
  const span = end - start;
  if (span <= 0) return start;
  if (loop === 'none') return Math.min(Math.max(raw, start), end);
...
const flattenedKeys = new WeakMap<TimelineTrack, Keyframe[]>();
 ...
 */
export function resolveKeyframes(track: TimelineTrack): Keyframe[] {
  const segments = track.segments;
  if (!segments || segments.length === 0) return track.keyframes;
  const cached = flattenedKeys.get(track);
  if (cached) return cached;
  const tagged: { key: Keyframe; order: number }[] = [];
...
export function interpolateKeyframes(
  ...
  b: Keyframe,
  time: number,
  easingOverride?: EasingName | CubicBezier,
): TrackValue {
  const span = b.t - a.t;
  ...
  const va = a.value;
  const vb = b.value;
  if (typeof va === 'number' && typeof vb === 'number') return lerp(va, vb, t);
  if (Array.isArray(va) && Array.isArray(vb)) return lerpArray(va, vb, t);
...
export function evaluateTrackAtProgress(
  track: TimelineTrack,
  p: number,
  options: EvaluateOptions = {},
): TrackValue | undefined {
