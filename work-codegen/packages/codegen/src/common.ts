...
/** Compile theme tokens to CSS custom properties for the critical CSS block. */
export function themeToCssVars(theme: ThemeTokens): string {
  ...
  for (const [step, t] of Object.entries(theme.typeScale)) {
    lines.push(`  --type-${step}-size: ${t.size};`);
    lines.push(`  --type-${step}-line-height: ${t.lineHeight};`);
    lines.push(`  --type-${step}-weight: ${t.weight};`);
  }
  for (const [name, value] of Object.entries(theme.spacing)) {
    lines.push(`  --spacing-${name}: ${value};`);
  }
  for (const [name, ms] of Object.entries(theme.motion.duration)) {
 ...
 */
export function ssrSkeleton(ir: SceneIR, config: EngineConfig): string {
  const parts: string[] = [];
  ...
  const visitRoots = (nodes: SceneIR['nodes'], depth: number): void => {
    for (const n of nodes) {
      ...
      if (n.kind === 'dom' && n.html) {
        ...
        out.push(
          `${pad}<div class="lumen-spatial" data-node="${escapeHtml(n.id)}" data-asset="${escapeHtml(n.assetId ?? '')}" role="img" aria-label="${label}"></div>`,
        );
      } else {
