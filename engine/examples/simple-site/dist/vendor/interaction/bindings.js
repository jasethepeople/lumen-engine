...
export function mapInputToOutput(value, inputRange, outputRange) {
...
export function snapValue(value, snap, threshold) {
...
export class BindingRuntime {
    ...
    feedInput(value) {
        if (isStaticFallback(this.binding.a11yFallback))
            return;
    ...
    update(dt, reducedMotion) {
        ...
        if (!smoothing || reducedMotion) {
            ...
            if (Math.abs(target - this.smooth.value) < 1e-4)
                this.smooth.value = target;
        }
        else {
