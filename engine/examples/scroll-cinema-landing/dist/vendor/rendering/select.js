...
export const FALLBACK_CHAIN = ['webgpu', 'webgl2', 'canvas2d', 'dom'];
/**
 ...
 */
export function selectRenderer(profile, preference) {
    const supported = {
 ...
 */
export async function createRenderer(backend, opts = {}) {
    let current = backend;
    let lastError = null;
    for (;;) {
        try {
            ...
            if (opts.surface !== undefined)
                await renderer.init(opts.surface);
            return renderer;
        }
        catch (err) {
...
async function construct(backend, opts) {
    switch (backend) {
        case 'dom':
