...
export interface PipelineContext {
  ...
  environment?: string;
  /** Version stamped into the deploy manifest (default: LUMEN_ENGINE_VERSION). */
...
export interface DeployManifest {
  ...
  generatedAt: string;
  /** Engine version that produced the build (additive; for cache busting/debug). */
  engineVersion?: string;
}

/** Version stamp written into the deploy manifest's engineVersion field. */
export const LUMEN_ENGINE_VERSION = '0.1.0';
  ...
  const files = await time('emit', async (): Promise<ArtifactFile[]> => {
    ...
    if (strategy.emitManifest) {
      const manifest: DeployManifest = {
        ...
        engineVersion: ctx.engineVersion ?? LUMEN_ENGINE_VERSION,
      };
