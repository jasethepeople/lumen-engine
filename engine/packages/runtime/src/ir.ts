...
import type { AssetManifest, ComposedScene, IRAssetRef, IRAssetVariant, IRNode, SceneIR, SceneNode } from '@lumen/contracts';
 ...
 */
export function describeSceneIRError(value: unknown): string | null {
  if (typeof value !== 'object' || value === null) return 'SceneIR must be an object';
  ...
  if (typeof ir.site !== 'object' || ir.site === null || typeof (ir.site as { id?: unknown }).id !== 'string') {
    return 'SceneIR requires site with a string id';
  }
  for (const key of ['nodes', 'tracks', 'bindings', 'assets'] as const) {
    if (!Array.isArray(ir[key])) return `SceneIR requires '${key}' to be an array`;
  }
  if (typeof ir.hydration !== 'object' || ir.hydration === null) return "SceneIR requires a 'hydration' object";

  // Collect node ids recursively; reject duplicates and malformed nodes.
  const nodeIds = new Set<string>();
  const walk = (nodes: readonly IRNode[], path: string): string | null => {
    for (const n of nodes) {
      ...
      nodeIds.add(n.id);
      if (!Array.isArray(n.children)) return `node '${n.id}' requires a 'children' array`;
      const err = walk(n.children, `${path}/${n.id}`);
      if (err) return err;
    }
    return null;
  };
  const nodeErr = walk(ir.nodes as readonly IRNode[], 'nodes');
  if (nodeErr) return nodeErr;

  // Tracks: ids unique, targets must resolve to a node in the graph.
  const trackIds = new Set<string>();
  for (const t of ir.tracks as readonly { id?: unknown; target?: unknown }[]) {
  ...
  for (const b of ir.bindings as readonly { id?: unknown; targetTrackId?: unknown }[]) {
  ...
  for (const a of ir.assets as readonly { id?: unknown; src?: unknown }[]) {
    if (typeof a?.id !== 'string' || a.id === '') return 'every asset requires a non-empty string id';
    if (typeof a.src !== 'string' || a.src === '') return `asset '${a.id}' requires a non-empty src url`;
  }
  return null;
}
...
function raiseNode(ir: IRNode): SceneNode {
  ...
  switch (ir.kind) {
    case 'dom':
      ...
      if (ir.anchor !== undefined) payload.anchor = ir.anchor;
      if (ir.rect !== undefined) payload.rect = ir.rect;
      if (ir.layerGroup !== undefined) payload.layerGroup = ir.layerGroup;
      break;
    case 'video-plane':
 ...
 */
function entryFromVariants(
  ref: IRAssetRef,
  variants: readonly IRAssetVariant[],
  base: { id: string; preload: NonNullable<IRAssetRef['preload']>; bytes: number },
): AssetManifest['assets'][string] | null {
  if (ref.kind === 'image') {
    const srcsetFor = (format: string): Record<number, string> | undefined => {
      const set: Record<number, string> = {};
      for (const v of variants) {
        if (v.format === format && typeof v.width === 'number') set[v.width] = v.src;
      }
        ...
        ...(mp4v
          ? {
              mp4: {
                url: mp4v.src,
                bytes: mp4v.bytes ?? 0,
                codec: codec === 'hevc' || codec === 'av1' ? codec : ('h264' as const),
              },
            }
          : {}),
